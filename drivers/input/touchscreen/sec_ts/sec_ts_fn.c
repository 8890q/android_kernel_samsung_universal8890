...
struct ft_cmd ft_cmds[] = {
	...
	{FT_CMD("singletap_enable", singletap_enable),},
...
static void singletap_enable(void *device_data)
{
	...
	if (!ts->use_sponge || ts->cmd_param[0] < 0 || ts->cmd_param[0] > 1)
		goto NG;

	if (ts->cmd_param[0])
		ts->lowpower_flag |= SEC_TS_MODE_SPONGE_SINGLE_TAP;
	else
		ts->lowpower_flag &= ~SEC_TS_MODE_SPONGE_SINGLE_TAP;
	...
	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_SPONGE_WRITE_PARAM, &data[0], 3);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: Failed to write mode %X\n", __func__, ts->lowpower_flag);
		goto NG;
	}

	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_SPONGE_NOTIFY_PACKET, NULL, 0);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: Failed to write notify packet\n", __func__);
		goto NG;
	}
	input_info(true, &ts->client->dev, "%s: %02X\n", __func__, ts->lowpower_flag);
